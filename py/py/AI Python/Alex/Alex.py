...
def open_application(text):
    if "google" in text:
        ...
        time.sleep(2)
        # Trong ngoặc là đường dẫn đến ứng dụng trong máy mình, các bạn tự tìm trong máy mình sao cho đúng nha
        os.startfile('C:\Program Files\Google\Chrome\Application\chrome.exe')
    elif "word" in text:
        speak("Mở Microsoft Word")
        time.sleep(2)
        # Ở đây cũng như ở trên
        os.startfile('C:\Program Files\Microsoft Office\Office15\WINWORD.EXE')
    elif "excel" in text:
        speak("Mở Microsoft Excel")
        time.sleep(2)
        # Ở trển cũng giống dưới này =))
...
def send_email(text):
    ...
    if 'nghĩa' in recipient:  # 'nghĩa' ở đây là keywords để máy tiếp tục gửi email cho bạn. Bạn có thể thay cái keywords này
        ...
        mail.ehlo()
        mail.starttls()
        # Ở khúc này, 'xyz' là địa chỉ email của bạn, 'abc' là mật khẩu của email đó
        mail.login('xyz', 'abc')
        mail.sendmail('xyz',
