...
namespace Teamwork.Web.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var studentNumber = await studentService.GetStudentNumberAsync(user.Id);
        ...
        public async Task<IActionResult> Index(IndexViewModel model)
        {
            ...
            var studentNumber = await this.studentService.GetStudentNumberAsync(user.Id);
            // if model returns student number
            if (!string.IsNullOrWhiteSpace(model.StudentNumber))
            {
                // If student has no record, we create a new one
                if (!string.IsNullOrWhiteSpace(studentNumber))
                {
                    ...
                    var result = await this.studentService.CreateAsync(student);
                }
                else
                {
                    // If number is different we update it
                    if (model.StudentNumber != studentNumber)
                    {
                        var result = await this.studentService.ChangeStudentNumberAsync(user.Id, model.StudentNumber);
                    }
                }
            }
        ...
        private string GenerateQrCodeUri(string email, string unformattedKey)
        {
            return string.Format(
                ...
                this.urlEncoder.Encode("Teamwork.Web"),
