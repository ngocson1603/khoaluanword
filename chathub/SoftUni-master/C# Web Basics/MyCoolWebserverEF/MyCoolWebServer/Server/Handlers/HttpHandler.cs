    ...
    public class HttpHandler : IRequestHandler
    {
        ...
        public IHttpResponse Handle(IHttpContext context)
        {
            try
            {
                ...
                var anonimourPaths = this.serverRouteConfig.AnonymousPaths;

                if (!anonimourPaths.Contains(context.Request.Path) &&
                    !context.Request.Session.Contains(SessionStore.CurrentUserKey))
                {
                    return new RedirectResponse(anonimourPaths.First());
                }
